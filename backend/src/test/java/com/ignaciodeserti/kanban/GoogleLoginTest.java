...
/**
 * The real OAuth2 redirect dance (browser -> Google -> callback) needs a live Google app
 * and isn't practical to exercise in CI, so these tests go straight at
 * AuthService.loginOrRegisterWithGoogle — the part GoogleOAuthSuccessHandler calls once
 * Spring Security has already done the OAuth2 handshake and verified the identity. What's
 * under test here is what happens *after* that point: account creation/linking, and the
 * password-optional account flows a Google-only sign-up creates.
 */
@SpringBootTest
...
class GoogleLoginTest {
    ...
    void firstGoogleSignInCreatesAVerifiedPasswordlessAccount() {
        AuthResponse session = authService.loginOrRegisterWithGoogle("newbie@example.com", "Newbie", true);
    ...
    void secondGoogleSignInReusesTheSameAccount() {
        ...
        AuthResponse second = authService.loginOrRegisterWithGoogle("repeat@example.com", "Repeat", true);
    ...
    void signingInWithGoogleVerifiesAnExistingUnverifiedLocalAccount() throws Exception {
        ...
        assertThat(userRepository.findByEmail("local-first@example.com").orElseThrow().isEmailVerified()).isFalse();
        ...
        assertThat(user.getAuthProvider()).isEqualTo(User.AuthProvider.LOCAL); // provider reflects signup origin
    }

    @Test
    void aPasswordlessAccountCanSetItsFirstPasswordWithoutProvingAnOldOne() throws Exception {
        AuthResponse session = authService.loginOrRegisterWithGoogle("set-pw@example.com", "Set PW", true);

        mvc.perform(post("/api/auth/change-password")
                        .header("Authorization", "Bearer " + session.token())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"currentPassword\":\"\",\"newPassword\":\"brand-new-123\"}"))
    ...
    void aPasswordlessAccountCanDeleteItselfWithoutAPassword() throws Exception {
        AuthResponse session = authService.loginOrRegisterWithGoogle("delete-me-google@example.com", "Bye", true);
