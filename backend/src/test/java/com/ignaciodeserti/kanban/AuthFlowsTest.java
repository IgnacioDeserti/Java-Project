...
/**
 * Covers the flows that require verification to actually be enforced, unlike
 * KanbanApiTest which runs with require-email-verification=false for convenience.
 *
 * Tokens are stored only as a SHA-256 hash (see UserTokenService), so tests recover the
 * raw token the same way a real user would: by reading it out of the email. Since no SMTP
 * is configured, EmailService logs it instead — this test attaches a Logback appender to
 * capture that line rather than trying to invert the hash.
 */
@SpringBootTest
...
class AuthFlowsTest {
    ...
    void refreshRotatesTheTokenAndLogoutRevokesIt() throws Exception {
        ...
        JsonNode refreshed = body(mvc.perform(post("/api/auth/refresh")
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void forgotPasswordResetsAndRevokesAllSessions() throws Exception {
        ...
        mvc.perform(post("/api/auth/reset-password")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"token\":\"" + resetToken + "\",\"newPassword\":\"new-secret-456\"}"))
                .andExpect(status().isOk());
