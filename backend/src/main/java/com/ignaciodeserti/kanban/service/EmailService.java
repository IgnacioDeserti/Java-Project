...
/**
 * Sends transactional emails (verification, password reset). When no SMTP host is
 * configured — the out-of-the-box local/dev setup — it logs the email instead of sending
 * it, so the app is fully usable without a mail provider. Plug in real SMTP credentials
 * (MAIL_HOST / MAIL_USERNAME / MAIL_PASSWORD) to send for real.
 */
@Service
@Slf4j
